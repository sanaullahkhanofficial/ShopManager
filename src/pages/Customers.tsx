  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
        <StatCard label="Total Customers" value={String(customers.length)} hint="Active Customers" icon={<Users size={18} />} tone="green" />
        <StatCard label="Active Shops" value={String(activeShops)} hint="Regular Buyers" icon={<Store size={18} />} />
        <StatCard label="Total Credit Limit" value={money(totalCreditLimit)} icon={<HandCoins size={18} />} tone="gold" />
        <StatCard label="Outstanding Balance" value={money(totalOutstanding)} icon={<Wallet size={18} />} tone="danger" />
        <StatCard label="New This Month" value={String(newThisMonth)} icon={<UserPlus size={18} />} />
      </div>

      <div className="flex flex-wrap items-center gap-2">
        <div className="relative min-w-[200px] flex-1">
          <Search size={15} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-stone-400" />
          <input className="input pl-9" placeholder="Search by name, shop, phone or CNIC…" value={query} onChange={(e) => setQuery(e.target.value)} />
        </div>
        ...
        <Button onClick={() => { setQuery(""); setTypeFilter(""); setAreaFilter(""); }}><RotateCcw size={14} /> Reset</Button>
        <div className="ml-auto flex flex-wrap gap-2">
          <Button onClick={() => setManageGroups(true)}><Tags size={14} /> Customer Groups</Button>
          <Button onClick={importCsv}><Upload size={14} /> Import (CSV)</Button>
          <Button onClick={exportCsv}><Download size={14} /> Export</Button>
          <Button variant="primary" onClick={resetForm}><Plus size={15} /> Add Customer</Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1fr_380px]">
        <div className="card">
          <DataTable
            ...
            pageSize={20}
            columns={[
              { header: "Customer Name", render: (r) => r.name },
              { header: "Shop Name", render: (r) => r.shop_name || "—" },
              { header: "Type", render: (r) => <span className="rounded-full bg-brand-green-50 px-2 py-0.5 text-xs font-medium text-brand-green-700">{r.customer_type}</span> },
              ...
              { header: "Action", render: (r) => (
                <div className="flex gap-2">
                  <button className="text-stone-400 hover:text-brand-green-700" title="View / Edit" onClick={() => editRow(r)}><UserIcon size={14} /></button>
                  <button className="text-stone-400 hover:text-brand-green-700" title="Ledger" onClick={() => (onOpenLedger?.(r.id), onNavigate?.("customerLedger"))}><BookText size={14} /></button>
        ...
        <div className="card space-y-3">
          {draft.id ? (
            <div className="flex items-center gap-3 border-b border-stone-100 pb-3">
              ...
              <div>
                ...
                <p className="text-xs text-stone-400">{draft.customer_type} Customer · Joined {draft.created_at ? formatDate(draft.created_at) : "—"}</p>
              </div>
            </div>
          ) : (
            <h3 className="text-sm font-semibold text-brand-navy-900">Add Customer</h3>
          )}

          {draft.id && stats && (
            <div className="grid grid-cols-2 gap-2 text-center text-xs">
              <div className="rounded-md bg-stone-50 p-2"><p className="text-stone-400">Credit Limit</p><p className="font-semibold">{money(draft.credit_limit)}</p></div>
              <div className="rounded-md bg-stone-50 p-2"><p className="text-stone-400">Current Balance</p><p className="font-semibold">{money(draft.balance)}</p></div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Shop Name" value={draft.shop_name || ""} onChange={(e) => setDraft({ ...draft, shop_name: e.target.value })} />
            <Field label="Customer Name *" value={draft.name || ""} onChange={(e) => setDraft({ ...draft, name: e.target.value })} />
            <Field label="Phone Number" value={draft.phone || ""} onChange={(e) => setDraft({ ...draft, phone: e.target.value })} />
            <Field label="WhatsApp" value={draft.whatsapp || ""} onChange={(e) => setDraft({ ...draft, whatsapp: e.target.value })} />
            <Field label="CNIC" value={draft.cnic || ""} onChange={(e) => setDraft({ ...draft, cnic: e.target.value })} />
            <SelectField label="Customer Type" value={draft.customer_type || "Retail"} onChange={(e) => setDraft({ ...draft, customer_type: e.target.value as Customer["customer_type"] })}>
              <option>Retail</option><option>Wholesale</option><option>Distributor</option>
            </SelectField>
            <Field label="City" value={draft.city || ""} onChange={(e) => setDraft({ ...draft, city: e.target.value })} />
            <Field label="Area / Location" value={draft.area || ""} onChange={(e) => setDraft({ ...draft, area: e.target.value })} />
            <SelectField label="Customer Group" value={draft.group_id || ""} onChange={(e) => setDraft({ ...draft, group_id: Number(e.target.value) || null })}>
              <option value="">None</option>
              {groups.map((g) => <option key={g.id} value={g.id}>{g.name}</option>)}
            </SelectField>
            <Field label="Credit Limit" type="number" value={draft.credit_limit ?? 0} onChange={(e) => setDraft({ ...draft, credit_limit: Number(e.target.value) })} />
            {!draft.id && <Field label="Opening Balance" type="number" value={draft.opening_balance ?? 0} onChange={(e) => setDraft({ ...draft, opening_balance: Number(e.target.value) })} />}
          </div>
          ...
          <div className="flex gap-2">
            ...
            <Button variant="primary" className="flex-1" onClick={save}><Save size={14} /> Save Customer</Button>
          </div>
          ...
          {draft.id && recentTx.length > 0 && (
            <div className="border-t border-stone-100 pt-2">
              <div className="mb-1 flex items-center justify-between">
                ...
                <button className="text-xs text-brand-green-700 hover:underline" onClick={() => (onOpenLedger?.(draft.id!), onNavigate?.("customerLedger"))}>View All →</button>
              </div>
              {recentTx.map((t) => (
          ...
          {draft.id && (
            <div className="flex flex-wrap gap-2 border-t border-stone-100 pt-3">
              <Button onClick={() => setPrintList(true)}><Printer size={14} /> Print Statement</Button>
              <Button onClick={() => push("info", "Not connected — SMS sending isn't set up yet (see Settings > Integrations)")}><MessageCircle size={14} /> Send SMS</Button>
      ...
      {printList && settings && (
        <PrintableList
          title="Customer List" settings={settings} rows={rows} keyField={(r) => r.id}
          columns={[
...
function GroupsModal({ groups, onClose, onChanged }: { groups: CustomerGroup[]; onClose: () => void; onChanged: () => void }) {
  ...
  return (
    <Modal title="Customer Groups" onClose={onClose}>
      <div className="mb-4 space-y-1">
        {groups.map((g) => <div key={g.id} className="rounded-md border border-stone-100 px-3 py-1.5 text-sm">{g.name}</div>)}
        {!groups.length && <p className="text-sm text-stone-400">No groups yet.</p>}
      </div>
      <div className="flex gap-2">
        <Field label="Group Name" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. VIP Wholesale" />
